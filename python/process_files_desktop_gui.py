...
def makeLatteButtonClick():
    updateGitHub()
makeLatteButton = ttk.Button(mainframe, text = "Update dashboard data", width = 30, command = lambda: makeLatteButtonClick() )
makeLatteButton.grid(column=4, row=15, sticky=W)

scrollingTextBox = tkst.ScrolledText(master=mainframe, width=120, height=30)
scrollingTextBox.grid(column=4, row=17, padx=8, pady=8)
scrollingTextBox.insert(END, 'Upload record:\n\n')
...
def updateGitHub():  # This is the function that is invoked when the button is clicked
    ...
    for sheet in wb:
        ...
        scrollingTextBox.insert(END, outFileNameRoot + '\n\n')
        scrollingTextBox.see(END)
        # causes updates to log window, see https://stackoverflow.com/questions/6588141/update-a-tkinter-text-widget-as-its-written-rather-than-after-the-class-is-fini
        # but I think maybe it only works on PC
